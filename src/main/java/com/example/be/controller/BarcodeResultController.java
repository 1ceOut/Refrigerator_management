    ...
    public List<Map<String, Object>> getRecipes(@RequestParam("ingredient") String ingredient) {
        ...
        try {
            ...
            for (JsonNode recipeNode : rowNode) {
                ...
                allRecipes.add(recipe);  // 모든 레시피를 저장
            }

            // 전체 레시피를 출력
            System.out.println("All Recipes: " + allRecipes);

            // 재료에 ingredient가 포함된 레시피만 필터링
            for (Map<String, Object> recipe : allRecipes) {
                String recipeIngredients = (String) recipe.get("RCP_PARTS_DTLS");

                if (recipeIngredients != null && recipeIngredients.contains(ingredient)) {
                    filteredRecipes.add(recipe);
                } else {
                    System.out.println("Filtered out: " + recipe.get("RCP_NM"));
                }
            }
